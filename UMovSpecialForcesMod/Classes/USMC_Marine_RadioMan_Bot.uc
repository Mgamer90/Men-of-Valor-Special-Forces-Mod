//////////////////////////////////////////////////////////////////////////////
//	File:	USMC_Marine_RadioMan_Bot.uc
//
//	Description	:	
//----------------------------------------------------------------------------
class USMC_Marine_RadioMan_Bot extends VietnamRadioManBot;

var VietnamPawn m_pMedicPatient;
var float m_medicPreviousHealingTime;
			...
			if(NamWeapon != None)
			{
				...
				if( m_pMedicPatientAmmo != None && m_pMedicPatientAmmo.AmmoAmount < m_pMedicPatientAmmo.MaxAmmo )
				{
					...
					else if( m_pMedicPatientAmmo.IsA('Ammo556NATO') )
					{
						...
						NamPickupAmmoHandgun = Spawn(class'PickupAmmo556NATO',Pawn,'',Pawn.GetBoneCoords('bip_LFoot').origin);
					}
					else if( m_pMedicPatientAmmo.IsA('Ammo762NATO') )
					{
						...
						NamPickupAmmoHandgun = Spawn(class'PickupAmmo762NATO',Pawn,'',Pawn.GetBoneCoords('bip_LFoot').origin);
					}
					else if( m_pMedicPatientAmmo.IsA('Ammo762NATOBeltFed') )
					{
						...
						NamPickupAmmoHandgun = Spawn(class'PickupAmmo762NATOBeltFed',Pawn,'',Pawn.GetBoneCoords('bip_LFoot').origin);
					}
					...
					else if( Pawn.IsA('ARVN') && m_pMedicPatientAmmo.IsA('Ammo762WP') )
					{
					...
					if(NamPickupAmmo != None)
					{
						NamPickupAmmo.AmmoAmount = m_RefillingAmount*NamPickupAmmo.AmmoAmount;

						m_ReserveAmmo = m_ReserveAmmo - NamPickupAmmo.AmmoAmount;
					...
					{
						Level.Game.Broadcast(Self, "I'm busy now!", 'Say');
					}
				}
				else
				{

...
class WeaponM16 extends VietnamWeapon;

var Material BayonetMaterial;
...
simulated function PostNetBeginPlay()
{
	...
	BayonetMaterial = Skins[2];

	// Make bayonet disappear
	// NOTE: This needs to be re-done in ApplyHandTexture()
	Skins[2] = Shader(DynamicLoadObject("Decals_tex.transparency_tex_shader", class'Shader'));
}

// overridden:  has a bayonet mode and can
// fire in bursts
//
// inputs:
// -- none --
...
state ModeSwitch
{
	ignores Fire, AltFire, NextWeapon, PrevWeapon;
	
	simulated function BeginState()
	{
		PlayAnim('bayonet_mode_switch', 1.0, 0.05,
				FIRING_ANIMATION_CHANNEL );
		PlaySound(WeaponSounds[EWeaponSound.EWS_ModeSelect]);
...
state FireEnd
{
	simulated function BeginState()
	{
		...
		{
			PlayAnim( 'bayonet_fire_end', 1.0, 0.0, 
				FIRING_ANIMATION_CHANNEL );
...
// overloaded:  uses special putaways when
...
// overloaded:  has a different idle when
// equipped with the bayonet
//
// inputs:
// -- none --
...
// applies the specified texture to
// the first person viewmodel
//
// inputs:
// inHandTexture - to use
//
// outputs:
// -- none --
